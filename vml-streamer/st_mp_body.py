	...
	def on_detection(self, result: vision.PoseLandmarkerResult, output_image: mp.Image, timestamp_ms: int):
		...
		for i, pose_landmarks in enumerate(result.pose_landmarks):
			...
			for lm_idx, lm in enumerate(world_points):
				...
				for axis in range(3):
					self.filtered_vals[f'{pose_name}{lm_idx}{axis}'] = lm[axis]	

					if self.apply_filter:
						if f'{pose_name}{lm_idx}{axis}' not in self.mp_data_filter.keys():
							self.mp_data_filter[f'{pose_name}{lm_idx}{axis}'] = OneEuroFilter(self.iteration, lm[axis], min_cutoff=self.one_euro_min_cutoff, beta=self.one_euro_beta)
							self.filtered_vals[f'{pose_name}{lm_idx}{axis}'] = lm[axis]							
						else:
							try:
								self.mp_data_filter[f'{pose_name}{lm_idx}{axis}'].beta = self.one_euro_beta
								self.filtered_vals[f'{pose_name}{lm_idx}{axis}'] = self.mp_data_filter[f'{pose_name}{lm_idx}{axis}'](self.iteration, lm[axis])
